    ...
    render(): React.ReactNode {
        ...
        return (
            <div className={cn(styles.select, this.props.className)} ref={this.getRef}>
                ...
                <List isShow={this.state.showList} list={this.props.selectList.filter(({ id }) => id !== selected)} onSelect={this.selectHandler}/>
            </div>
        );
    }
}
...
const List = ({ list, onSelect, isShow }) => {
    return !isShow ? null : <div className={styles.list}>
        {
            list.map(item => (
                <div key={item.id} className={styles.listItem} onClick={(e) => onSelect(item)}>{item.text}</div>
            ))
        }
    </div>
};
