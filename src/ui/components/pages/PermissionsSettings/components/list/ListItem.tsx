...
export const ListItem: React.FunctionComponent<IParams> = ({ className, originName, permissions, showSettings, toggleApprove, permissionsText }) => (
    <div className={cn(className, styles.permissionItem, {[styles.approved]: permissions.includes('approved')})}>
        <div>{originName}</div>
...
const ItemButton: React.FunctionComponent<any> = ({ permissions, onClick }) => {
    ...
    return <Button type={BUTTON_TYPE.TRANSPARENT}
