...
import { getAccountLink, getAssetDetailLink, getCollectionLink } from 'ui/urls';
...
export function NftInfo() {
  ...
  const creatorUrl = networkCode && getAccountLink(networkCode, nft?.creator);
  const nftUrl =
    nft && networkCode
      ? getAssetDetailLink(networkCode, nft.assetId ?? nft.id, nft?.tokenId)
      : undefined;

  const collectionUrl =
    networkCode && getCollectionLink(networkCode, nft?.assetId);
  return (
    <div className={styles.root}>
      ...
      <div className={styles.content}>
        ...
        {currentBlockchainType === BLOCKCHAIN_TYPES.UNIT0 && nft?.assetId && (
          <>
            <div className={styles.fieldName}>Collection Address</div>
            <div className={styles.fieldContent}>
              <div>
                <Ellipsis text={nft.assetId} size={16} />
              </div>
              {currentBlockchainType === BLOCKCHAIN_TYPES.UNIT0 && (
                <div>
                  <Tooltip content={t('nftInfo.creatorUrlTooltip')}>
