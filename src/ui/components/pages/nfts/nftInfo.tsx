...
export function NftInfo() {
  ...
  const params = useParams<{ assetId: string; tokenId?: string }>();
  ...
  // Extract contract address and token ID from assetId
  const contractAddress = params.assetId;

  const assetId = params.tokenId
    ? `${params.assetId}_${params.tokenId}`
    : params.assetId;
  const nftInfo = usePopupSelector(state => {
    return state.nfts?.[assetId as string];
  });
  ...
  if (nft) {
    switch (networkCode) {
      case '88817':
        nftUrl = `https://explorer-testnet.unit0.dev/token/${contractAddress}/instance/${tokenId}`;
        break;
      case '88811':
        nftUrl = `https://explorer.unit0.dev/token/${contractAddress}/instance/${tokenId}`;
        break;
      default:
        getAssetDetailLink(networkCode!, nft.id);
    }
  }
  ...
  return (
    <div className={styles.root}>
      <div className={styles.header}>
        <div className={styles.title}>{!nft ? <Loader /> : title}</div>
      </div>
      ...
      <div className={styles.content}>
        ...
        {/* Display collection address if available */}
        {(contractAddress || (nft as any)?.collectionAddress) && (
          <>
            <div className={styles.fieldName}>Collection Address</div>
            <div className={styles.fieldContent}>
              <div>
                <Ellipsis
                  text={contractAddress || (nft as any).collectionAddress}
                  size={16}
                />
              </div>
              {currentBlockchainType === BLOCKCHAIN_TYPES.UNIT0 && (
                  ...
                  <Tooltip content={t('nftInfo.creatorUrlTooltip')}>
                    {props => (
                      ...
                      <a
                        ...
                        href={
                          networkCode === '88817'
                            ? `https://explorer-testnet.unit0.dev/token/${
                                contractAddress ||
                                (nft as any).collectionAddress
                              }`
                            : `https://explorer.unit0.dev/token/${
                                contractAddress ||
                                (nft as any).collectionAddress
                              }`
                        }
        ...
        {(nft as any)?.metadata && (
          <>
            <div className={styles.fieldName}>Metadata</div>
            <div className={styles.fieldContent}>
              <pre style={{ whiteSpace: 'pre-wrap', fontSize: '12px' }}>
                {JSON.stringify((nft as any).metadata, null, 2)}
              </pre>
            </div>
          </>
        )}
