...
interface EncryptedKeystore {
    decrypt: (
        ...
        ImportKeystoreNetwork,
        { accounts: Array<{ address: string; name: string; networkCode: string; seed: string }> }
    >;
}
...
export const ImportKeystore = connect(
    mapStateToProps,
    actions
)(function ImportKeystore({ allNetworksAccounts, batchAddAccounts, setTab }: Props) {
                ...
                onSubmit={async (keystoreFile, password) => {
                    ...
                    try {
                        ...
                        Object.entries(newProfiles).forEach(([network, profile]) => {
                            const currentNetworkAccounts = allNetworksAccounts.filter((acc) => acc.network === network);
