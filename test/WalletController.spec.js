...
describe('WalletController', () => {
  ...
  it('Should not init vault without password', () => {
    const controller = new WalletController();
    expect(() => controller.initVault()).to.throw(
      'Password is needed to init vault'
    );
  });
  ...
  it('Should not set new password with invalid old pass', () => {
    const controller = new WalletController({ initState });
    expect(() => controller.newPassword('bad pass', 'newPassword')).to.throw(
      'Invalid password'
    );
  });
  ...
  it('Should not set new password with no new password', () => {
    const controller = new WalletController({ initState });
    expect(() => controller.newPassword(undefined, 'newPassword')).to.throw(
  ...
  it('Should export account', () => {
    ...
    const exported = controller.exportAccount(
      controller.wallets[0].getAccount().address,
      password
    );
  ...
  it('Should not export on invalid password', () => {
    ...
    const exported = () =>
      controller.exportAccount(controller.wallets[0].getAccount().address);
  ...
  it('Should sign arbitrary bytes', async () => {
    ...
    expect(
      SG.utils.crypto.isValidSignature(
