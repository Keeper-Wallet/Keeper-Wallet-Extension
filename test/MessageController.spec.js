...
describe('MessageController', () => {
  ...
  const assetInfoController = new AssetInfoController({
    getNetwork: () => 'testnet',
  ...
  beforeEach(() => {
    controller = new MessageController({
      signTx: async () => 'placeholder',
      broadcast: async () => 'broadcast placeholder',
      assetInfo: id => assetInfoController.assetInfo(id),
    });
  });
  ...
  it("Shouldn't add invalid messages to pipeline", () => {
    return controller.newMessage({}, 'transaction', origin, account).should
      .eventually.be.rejected;
  });

  it('Should approve message', async () => {
    const messageId = await controller.newMessage(
      tx,
      'transaction',
      origin,
      account
  ...
  it('Should reject messages', async () => {
    ...
    return messageResultPromise.should.eventually.be.rejectedWith(
      'User denied message'
    );
  });
});
