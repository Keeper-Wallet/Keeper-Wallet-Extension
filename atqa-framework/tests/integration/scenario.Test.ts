...
Data(initData).Scenario('Update unlogged keeper', async ({ current }) => {
  ...
  const extId = await basePage.getItemFromLocalStorage(EXT_ID);
    ...
    await I.waitForElement(modalPage.SELECTORS.SETTINGS_BUTTON, clockUnit.SECONDS * 30);
    await I.click(modalPage.SELECTORS.SETTINGS_BUTTON);
    await I.waitForElement(modalPage.SELECTORS.SETTINGS_MENU.GENERAL_SETTINGS, clockUnit.SECONDS * 30);
    await I.click(modalPage.SELECTORS.SETTINGS_MENU.GENERAL_SETTINGS);
    await I.waitForElement(modalPage.SELECTORS.SETTINGS_MENU.OPEN_TIMEOUT_MENU, clockUnit.SECONDS * 30);
    await I.click(modalPage.SELECTORS.SETTINGS_MENU.OPEN_TIMEOUT_MENU);
    await I.waitForElement(modalPage.SELECTORS.SETTINGS_MENU.SET_TIMEOUT('1 hour'), clockUnit.SECONDS * 30);
...
Data(initData).Scenario(
  ...
  async ({ current }) => {
    ...
    await I.click(modalPage.SELECTORS.TRANSACTION_MODAL.CLOSE_TRANSACTION_WINDOW);
    await I.waitForElement(assetPage.SELECTORS.WAVES_AMOUNT(16), clockUnit.SECONDS * 30);
...
Data(initData).Scenario('Update keeper during the account seeding', async ({ current }) => {
    const {
        extensionTag: EXT_ID,
        updateDir: UPDATE_DIR,
        locator: UPDATE_BUTTON_SELECTOR,
    ...
    await I.click(modalPage.SELECTORS.SUBMIT_KEEPER_PASSWORD);
    await I.waitForElement(modalPage.SELECTORS.ACCOUNTS.CHOOSE_ACCOUNT_BUTTON, clockUnit.SECONDS * 30);
    await I.forceClick(modalPage.SELECTORS.ACCOUNTS.CHOOSE_ACCOUNT_BUTTON);
    await I.wait(clockUnit.SECONDS * 5);
    await I.waitForElement(modalPage.SELECTORS.ACCOUNTS.ACCOUNT_AT_POSITION(1), clockUnit.SECONDS * 30);
